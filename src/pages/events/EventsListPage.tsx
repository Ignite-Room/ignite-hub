...
function PriceBadge({ startingPriceInPaise }: { startingPriceInPaise: number | null }) {
    if (startingPriceInPaise === null) return <Badge variant="outline" className="text-xs">No tickets yet</Badge>;
    if (startingPriceInPaise === 0) return <Badge variant="outline" className="text-xs text-emerald-400 border-emerald-400/30">Free</Badge>;
    return <Badge variant="outline" className="text-xs">From {formatRupees(startingPriceInPaise)}</Badge>;
}
        ...
        >
            ...
            <div className="p-5 space-y-3 flex-1 flex flex-col">
                ...
                <div className="mt-auto space-y-1.5 pt-2 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" />{event.registrationCount} registered</div>
                </div>
                <div className="flex items-center gap-2 pt-1">
                    {event.organizer.logoUrl ? (
                        <img src={event.organizer.logoUrl} alt={event.organizer.orgName} className="w-5 h-5 rounded-full object-cover flex-shrink-0" />
                    ) : (
                        <div className="w-5 h-5 rounded-full bg-secondary flex-shrink-0" />
                    )}
                    <p className="text-xs text-muted-foreground">By {event.organizer.orgName}</p>
                </div>
            </div>
        </Link>
    );
